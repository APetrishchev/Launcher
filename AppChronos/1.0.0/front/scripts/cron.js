...
export class Cron {
  static async getInstance(kvargs) {
    ...
    let res = await fetch("/home/.application/Chronos/data.json")
    res = await res.json()
    debug("Cron", res)
    if (!res.TalkClock.disable) {
      instance.talkClock = new TalkClock({
        path: `${res.TalkClock.path}/${kvargs.lang}/${res.TalkClock.gender}/`,
        preSound: res.TalkClock.preSound, volume: res.TalkClock.volume
      })
    }
    const list = []
    for (const row of res.CronList) {
    ...
    instance.db = kvargs.db
    instance.db.init(list)
    return instance
  }
  ...
  async hasEvent(date1, date2, kvargs) {
    const result = await this.db.get("cron", kvargs.key, kvargs.val)
...
console.log(`  minutes: x${binPosCode.minutes.toString(16)}`, BinPosCode.posBinCode(binPosCode.minutes, 60))
    ...
    const results = await this.db.get("cron", kvargs.key, kvargs.val)
    let idx = 0
