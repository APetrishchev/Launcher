...
export class Obj {
  ...
  static addEventListeners = (owner, element, events) => {
    for (const [event, evnLstnr] of Object.entries(events)) {
      element.addEventListener(event, evn => evnLstnr(owner || element), false) }
  }
...
export class Gauge extends Obj {
  set value(val) {
    ...
    this.percentsElement.append(`${val} %`)
    this.needleElement.setAttribute("style", `--val: ${140 + 2.6 * val}deg`)
  }

  set text1(val) {
    this.textElement1.append(val)
  }

  set text2(val) {
    this.textElement2.append(val)
  }

  set text3(val) {
    this.textElement3.append(val)
  }
...
export class ProgressBar extends Obj {
  ...
  set text(txt) {
    this.progressElement.append(txt)
  }
  ...
  show() {
    ...
    for (const field of this.fields) {
      if (field[0]) {
        ...
        const layout = this.add(Obj.createElement({ classList: Obj.combine(this.classList, "-Fields"),
