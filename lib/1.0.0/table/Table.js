...
// 						await this.#tabShow()
		...
		if (false) {
			this.toolBar = new ToolBar({
				parent: this.element,
				toolSetIds: ["toolSetMain"]
			})
			if (this.behaviors & Table.RowDelete) {
				this.toolBar.toolSetMain.addTool(new Button({
					id: "deleteButton",
					classList: ["button", "deleteButton"],
					disabled: 1,
					events: {"click": evn => {
						evn.preventDefault()
						// btn.disabled: 1
			...
			if (this.behaviors&Table.RowAdd) {
				this.toolBar.toolSetMain.addTool(new Button({
					id: "addButton",
					classList: ["button", "addButton"],
					events: {"click": async evn => {
						evn.preventDefault()
