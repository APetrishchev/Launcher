...
export class Form extends Obj {
  ...
  constructor(kvargs = []) {
    ...
    if (this.behaviors & Form.CanselButton) {
      this.addButton("CanselButton", Button.Cansel()) }
  }
  ...
  show() {
    super.show()
    this.titleElement = Obj.createElement({ parent: this.element, classList: Obj.combine(this.classList, "-Title"),
      children: this.title })
    this.buttonsElement = Obj.createElement({ parent: this.element, classList: Obj.combine(this.classList, "-Buttons"),
